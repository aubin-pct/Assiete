...
- État global : `S` (données persistées), `view` (onglet courant), `curDate` (jour affiché, `YYYY-MM-DD` en heure locale), `draft` (repas en cours de saisie), `armedDelete` (confirmation en deux temps, utilisée seulement pour « Tout effacer »).
...
- Suppression de photo : `[data-rmphoto]` retire une photo du brouillon ; `[data-delphoto]` retire la miniature d'un repas enregistré, avec « Annuler » dans le toast.
- Suppression d'un repas (`[data-del]`) : immédiate, avec « Annuler » pendant 5 s qui le remet à sa place d'origine.
- Dictée (section « dictée » du JS) : bouton `#mic` dans le champ « Quantités et précisions », affiché seulement si le navigateur a `SpeechRecognition`/`webkitSpeechRecognition`. Français, une phrase par appui ; le texte s'ajoute à la note (`noteChanged()` met à jour le bouton Analyser). Arrêtée en quittant l'écran d'ajout (`stopDictation`). La transcription passe par les serveurs de Google ou d'Apple.
- Bilan de la semaine (`weekHTML`, en haut de l'historique) : 7 derniers jours sans aujourd'hui ; moyennes calories et protéines sur les jours complets (≥ `MIN_DAY_KCAL`), écart avec les 7 jours d'avant, pastilles par jour (dans l'objectif à ±10 %, au-dessus, en dessous, non noté), jours avec protéines ≥ 95 % de l'objectif, jours notés, série de jours consécutifs avec au moins un repas.
...
- Pas de `alert`, `confirm` ni `prompt`. Une action réversible s'exécute tout de suite avec « Annuler » dans le toast (`toast(msg, { label, fn })`) ; seule une action massive (« Tout effacer ») garde la confirmation en deux temps via `armedDelete`.
